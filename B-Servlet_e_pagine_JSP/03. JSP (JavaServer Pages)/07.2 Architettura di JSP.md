...
- Quando il server riceve una richiesta per una JSP:
  1. Il container traduce la JSP in una Servlet Java (solo alla prima richiesta o dopo una modifica).
  2. La Servlet viene compilata in bytecode.
...
- Performance leggermente inferiori rispetto alle Servlet pure per la prima richiesta (traduzione/compilazione).

## Best practice
- Separare la logica di presentazione dalla logica applicativa (usare le Servlet come controller).
